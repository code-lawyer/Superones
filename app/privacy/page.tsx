...
export default function PrivacyPage() {
  return <ProsePage code="PRIVACY / DATA MINIMIZATION" title="只收集完成当前任务所需的数据。" lead="公开浏览不要求创建账户；需要联系、付款核验或业务处理时，页面会在提交前说明具体用途。" sections={[
    ...
    { title: "OPC 服务订单", paragraphs: ["下单时收集联系人姓名，以及手机号、邮箱或即时通讯账号中的至少一种。联系方式只用于订单联系、到账核验和后续交付，不公开，也不进入境外采集任务。订单完成、取消或退款满 24 个月后清除联系方式；订单号、服务与价格快照、付款交易号和必要的核验时间作为最小业务记录保留。", "Vault2077 不收集付款账户登录信息、支付密码、银行卡号或付款验证码。付款在独立付款页面完成；Vault2077 仅接收经过验证的付款状态通知和交易查询结果。"] },
