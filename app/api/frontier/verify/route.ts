...
import { challengeMatches, CURRENT_SEASON, getSubmission, markSubmissionVerified } from "@/lib/frontier-store";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (payload.platform !== "vault2077" || payload.season !== CURRENT_SEASON.code || payload.repository !== submission.repository || typeof payload.challenge !== "string" || !challengeMatches(payload.challenge, submission.challengeHash)) {
    ...
    return NextResponse.json({ repository: verified.repository, baselineStars: verified.baselineStars, verifiedAt: verified.verifiedAt });
  } catch (error) {
