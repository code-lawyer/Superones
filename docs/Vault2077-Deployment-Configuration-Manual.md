...
事务邮件推荐使用阿里云邮件推送的独立子域 `notify.superones.top`，发件地址与 SMTP 用户统一为 `orders@notify.superones.top`，SMTP 主机为 `smtpdm.aliyun.com`、端口为 465；这样不改动根域现有飞书 MX。`VAULT2077_SMTP_PASSWORD` 必须填写邮件推送控制台生成的 SMTP 密码，不得填写阿里云登录密码。若改用根域飞书公共邮箱，则主机使用 `smtp.feishu.cn:465`，用户名与 From 必须是同一个已启用 SMTP 的 `@superones.top` 公共邮箱，密码使用专用客户端密码。回复地址和负责人通知仍固定为 `lanzhouda@163.com`。
