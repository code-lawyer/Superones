...
两个配置都只在境内持有凭证，均须记录提供方、模型、提示和处理版本。普通结构化编辑固定显式发送 `thinking.type=disabled`；只有任务合同明确要求推理时才发送 `thinking.type=enabled`，可选强度使用 Chat Completions 顶层 `reasoning_effort`，不得使用非官方的嵌套 `reasoning` 字段。生产基线为 `vault_editorial=300`、`sic_editorial=200`，也可显式设为 `unlimited`；无论哪种额度都必须使用有界并发、超时、连续三次瞬时失败后 60 秒熔断及 inbox 指数退避。worker 单轮固定使用 45 分钟处理预算；达到截止时间后停止领取新批次，SiC 无效响应的本地重试和拆批也必须退出并让事务回滚，不能依靠放大 systemd 超时吸收故障。DNS、TLS、HTTP、额度耗尽、时间预算耗尽或无法取得成功 completion 属于编辑基础设施故障：整个数据库事务回滚，批次进入 `retryable`，不得被内容隔离误标为 `processed`。成功 completion 的正文不是约定 JSON/Schema 时先原地重试一次；批量响应仍无效则降级为逐条编辑，单条响应仍无效才隔离该条并允许同批合格记录发布。rankings 以及 Frontier 的确定性核验、观察、排名和结算不得使用编辑模型。
...
付款通知收件人不从浏览器或订单请求读取，固定复用生产管理员身份 `lanzhouda@163.com`。邮件只含订单号、服务、金额、付款时间、交易号和受保护后台链接，不发送客户姓名、手机号、邮箱或地址。支付宝回调只原子写入订单、付款凭证和 outbox 后立即应答，独立的 `vault2077-opc-order-maintenance.timer` 每分钟发送并重试。开放前必须完成一次真实 TLS 邮件投递、SMTP 暂时失败后的 outbox 重试和重复事件不重复发送测试。付款凭证上的我方名称、统一社会信用代码、规范网址和 ICP 备案号分别来自服务器锁定的法律档案与公开来源配置；缺少或不一致时纸质付款入口关闭。

163 邮箱配置中，`VAULT2077_SMTP_USER` 与 `VAULT2077_SMTP_FROM` 填写已开启 SMTP 服务的发件邮箱；`VAULT2077_SMTP_PASSWORD` 必须填写网易邮箱生成的客户端授权码，不得填写网页登录密码。付款通知的固定收件地址仍为 `lanzhouda@163.com`，与发件邮箱配置相互独立。

### OPC e 签宝电子签约

| 变量 | 说明 |
| --- | --- |
| `VAULT2077_OPC_ESIGN_ENABLED` | 未来电子签约独立开关；当前保持 `false`，不得影响纸质签约订单 |
...
以下配置和流程只用于未来重新启用电子签约，不是当前纸质订单上线依赖。模板后续可在 e 签宝后台上传；重新开放电子签前必须按 `docs/research/opc-esign-integration-operator-checklist.md` 使用稳定控件名、用官方拖章工具确认平台方签章坐标，并回填两个模板 ID、版本和两组坐标。项目先调用 `/v3/files/create-by-doc-template` 生成合同，再调用 `/v3/sign-flow/create-by-file` 发起流程；流程 ID 立即绑定到订单后才调用 `/v3/sign-flow/{signFlowId}/sign-url` 获取免登录托管页面。回调执行 HmacSHA256 验签并按事件摘要幂等记录信号，返回页或受保护后台再主动查询、验签和归档。该电子流程不得被纸质订单路由调用。

专用合同 Bucket 必须关闭公共读和浏览器直连，启用 10 年保留锁/WORM，并使用与公开媒体不同的 RAM 身份。应用写入内容哈希不可变 key，后台下载时由应用服务器读取并复核 SHA-256；管理员看到“下载已签合同”和“导出客户联系方式”两个独立操作，均要求最近 5 分钟再认证并写审计。未付款取消订单满 90 天清除联系人和地址；已付款订单完成或退款满 24 个月后清除非合同联系方式和地址；已签合同与验签/存证证据保留 10 年。
...
GitHub Actions 只持有接收 URL、公开任务 URL、公开任务只读密钥、版本化签名密钥环和活动 key ID，不得持有 worker、后台、用户数据或境内 LLM 密钥，也不得调用 `/api/internal/acquisition/process`。每次采集必须上传 manifest、报告、不可变批次、SHA256 与验证授权标记；artifact 路径必须使用这四项白名单，不得上传 collector 临时目录，证据缺失必须使 workflow 失败。境内安装并启用 `vault2077-acquisition-worker.{service,timer}`、`vault2077-healthcheck.{service,timer}` 与 `vault2077-opc-order-maintenance.{service,timer}`：前者每五分钟消费 inbox、执行到期重试并清理保留期外记录，健康探针每五分钟把受保护业务 health 转换为 systemd 退出码，OPC 维护任务每分钟发送付款邮件 outbox 并执行联系方式保留清理；Frontier 在白天每两小时执行 `npm run frontier:tick`。这些任务都必须配置失败告警和错过任务补跑策略。
