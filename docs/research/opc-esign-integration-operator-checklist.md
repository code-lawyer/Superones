...
3. 运维把两家沙箱凭证写入未跟踪的 `.env.local` 后运行 `npm run opc:verify-providers`。该只读探针会查询两套 e 签宝模板并核对全部 OPC 控件 Key，同时用保留订单号调用支付宝交易查询；它不会创建签署流程、支付订单或资金交易，也不会输出密钥。
4. Codex 独立完成沙箱 PoC：模板填充 → 企业自动章 → 自然人签署 → H5 → 回调/查询 → 下载验签归档。
5. 用户亲自验收合同显示效果、实名步骤和盖章主体；管理员/法定代表人完成必要的印章授权。
6. Codex 实现完整 OPC 状态机、后台、异常补偿和测试。
7. 用户完成购买和套餐生效、正式应用/模板/印章配置，并把生产密钥注入部署环境。
8. Codex 运行生产前只读检查和小流量真实签署验收；用户批准后打开功能开关。
