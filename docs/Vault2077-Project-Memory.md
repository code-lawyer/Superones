...
ADR-0023 将游骑兵顾问身份纳入同一 OPC 目录事实源：身份使用稳定 ID、可修改公开名称和类别说明，首次发布后 ID 锁定；后台可以新增、改名、修改说明、排序和删除空身份，仍被顾问引用的身份必须先迁移或移除。三个基础设施分组和六个专项领域也在同一目录保存受控公开说明，后台只能修改说明，不能改变分类集合、名称或顺序。目录 schema v2 同时保存 `serviceCategoryDescriptions`、身份集合和顾问 `identityId`；读取层迁移 schema v1 身份，并对说明扩展前的既有 v1/v2 草稿、当前发布目录和历史快照确定性补全初始说明，完整后台请求缺少分类说明、身份数组或身份说明时失败关闭。公开 OPC 目录的跨请求缓存键随公开投影结构显式版本化，新增必填投影字段时必须提升键版本，不能让新组件复用旧结构缓存。为保持上一 release 可读，状态序列化在回滚窗口内派生非权威 legacy `identity` 影子值，当前 API 与领域逻辑不读取它，回滚期间冻结 OPC 后台写入；公开说明继续存放在 PostgreSQL `opc-service-catalog` JSONB 文档，头像 OSS 只存媒体对象，不新增平行数据库字段或第二事实源。
