...
- `/sic` 是单一连续聚合页：依次呈现论文主栏、平台趋势榜以及档案、课程和播客；首次响应只发送当前公开快照各组的轻量首批投影，后续以各组独立的首屏快照 ID 每次读取五条，该组快照变化时要求刷新而不混合结果，单组读取失败不阻断其他组；稳定锚点、榜单 `board` 查询参数和原页分批展开保留定位能力，不再要求用户切换独立视图。
...
ADR-0023 将游骑兵顾问身份纳入同一 OPC 目录事实源：身份使用稳定 ID 和可修改公开名称，首次发布后 ID 锁定；后台可以新增、改名、排序和删除空身份，仍被顾问引用的身份必须先迁移或移除。目录 schema v2 同时保存身份集合和顾问 `identityId`，读取层只在显式 schema v1 上迁移生产既有草稿、当前发布目录和历史快照；v2 或后台请求缺少身份数组时失败关闭。为保持上一 release 可读，状态序列化在回滚窗口内派生非权威 legacy `identity` 影子值，当前 API 与领域逻辑不读取它，回滚期间冻结 OPC 后台写入；不新增平行数据库字段或第二事实源。
