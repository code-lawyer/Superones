...
> 现行决策（ADR-0017，2026-08-02 修订）：Follow Builders 是明确授权的例外，X/Blogs/Podcasts 三个中央 feed 默认可信且不经过本地上游准入审核；本文其余内容是 2026-07-22 的历史技术调研。

> 当前决策（ADR-0017）：Horizon 只作为获准来源的原始读取适配层；Hacker News/Lobsters 已退出运行时。机构混合 Feed 仍不能依靠采集后 LLM 分类取得生产准入。
