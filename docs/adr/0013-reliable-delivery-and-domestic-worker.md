...
8. 编辑提供方的配置、DNS/TLS/HTTP、预算、请求额度或无法取得成功 completion 的协议故障不得降级为单条内容 quarantine；此类基础设施故障必须让业务事务回滚并由 inbox 自动重试。已经取得成功 completion、但模型正文不是约定 JSON/Schema 时，先原地重试一次；批量响应仍无效则降级为逐条编辑，单条响应仍无效则只隔离该条并提交同批其他合格记录。
