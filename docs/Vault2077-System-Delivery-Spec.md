...
Redis 仅在需要分布式锁、缓存或队列吞吐时加入。生产首版按 ADR-0016 为公开游骑兵头像配置公开媒体 OSS；按 ADR-0018 为 OPC 已签合同配置权限和凭证完全分离的私有归档 OSS。采集原始包、授权材料和其他长期归档仍不得进入这两个 Bucket。
...
- OPC 订单先签署后付款：公开入口保存加密签约方；恢复令牌由独立可轮换密钥派生，订单只保存摘要、非秘密 nonce、密钥 ID 和 90 天有效期。身份认证、意愿认证与签字/盖章只在 e 签宝托管顶层页面进行。签署回调使用原始请求体 HmacSHA256 验签、限制时间窗并按事件摘要幂等记录；回调和浏览器返回都不能直接放行付款，必须主动查询绑定流程，确认双方及签署区完成，下载 PDF、验签、计算 SHA-256 并成功写入专用私有 OSS 后才进入待付款。未归档订单意外到账进入异常状态。生产必须同时启用电子签约与付款开关，且禁止模拟供应商。
