...
- OPC 新订单按 ADR-0020 在线确认服务订单及线下对公转账协议：公开页同时展示一个已发布修订内的企业账户、协议 PDF 和联系人二维码，线上付款禁用，线下付款生成固定金额订单及唯一付款附言，不跳第三方收银台。订单冻结服务/协议正文、付款资料修订、账户快照及 PDF/二维码哈希，联系人和付款户名使用数据密钥环加密；恢复令牌只保存摘要、nonce、密钥 ID 和有限有效期。付款资料修订不可静默覆盖，资产请求的 SHA-256 必须与当前修订匹配且响应使用 `no-store`；PDF 以内联方式支持协议弹窗预览，同源下载链接仍可显式下载。应用和公网边缘只允许协议 PDF 的精确路径使用 `X-Frame-Options: SAMEORIGIN`，二维码与其余页面继续使用 `DENY`。订单 JSON 与支付宝通知均按流式累计字节限制读取，不先无界缓冲完整正文。`awaiting_payment -> paid` 仅由最近五分钟再认证的后台提交固定金额、付款户名、唯一银行流水号、入账时间、预期订单版本和服务端强制的逐项核对确认驱动；流水号在判重、保存和审计前规范化，账户、银行、支行、账号、付款附言及 PDF/二维码摘要任一快照缺失均拒绝，状态与审计同一事务提交，审计只存流水指纹。尚未确认到账的银行订单可由相同再认证门禁取消并进入 90 天联系人清理。首次核验到账生成唯一付款凭证和 `payment_confirmed` outbox，凭证标识线下对公转账；用户在下单成功后及同一浏览器刷新后均有状态/凭证入口。`lib/opc-orders/` 深模块隔离 `bank_transfer` 与 `alipay` 证据，银行订单不得触发支付宝查询、通知或退款；`VAULT2077_OPC_PAYMENTS_ENABLED=false` 只禁止新支付宝付款请求，既有 ADR-0019 订单继续使用已配置身份完成验真、查询、关单和退款。线下退款自动闭环未完成前不得以普通状态写入冒充退款证据。付款资料发布通过固定 VPS 暂存目录和 `opc:publish-offline-payment-profile` 写入 PostgreSQL；公开路由不直接读文件或 OSS。
