...
除事件簿外，公开信息全部采用逐来源当前快照：新成功快照替换旧快照，不保留历史；失败来源继续展示上一成功快照并标记延迟。来源不再处于当前运行注册表的 approved 集合时，部署该修订后的下一轮处理必须清除其公开快照和运行报告；事件簿内已固定的证据继续保留。单条处理失败不得阻塞同批其他条目；同一来源同轮分片按最差状态聚合，不能由后到成功分片覆盖失败。事件簿永久保存综合结果及形成事件时使用的不可变资讯证据副本，不依赖当前资讯瀑布继续保留原条目。
...
Redis 仅在需要分布式锁、缓存或队列吞吐时加入。生产首版按 ADR-0016 强制为公开游骑兵头像配置 OSS；原始包与归档仍不进入对象存储，除非后续 ADR 证明其必要性。
