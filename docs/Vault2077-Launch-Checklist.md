...
- [ ] OPC 签约与付款采用成对功能开关：未接入时 `VAULT2077_OPC_ESIGN_ENABLED=false` 且 `VAULT2077_OPC_PAYMENTS_ENABLED=false`，不渲染表单、不收集签约信息且 API 返回 503；开放前 e 签宝自然人/组织模板、法定代表人签署、回调验签、双方/签署区主动查询、PDF 下载验签、专用私有 OSS 归档和支付宝支付全链路均已完成沙箱与生产演练。
...
- [ ] 后台只能按允许的状态机处理待签署、待付款、到账异常、已到账、已完成、已取消和已退款订单；签署完成只采信 e 签宝主动查询并要求验签归档成功，付款只采信支付宝真实到账记录；后台查询、状态变更、合同下载和联系方式导出要求最近五分钟再认证并写入审计。
- [ ] 订单完成、取消或退款满 24 个月后的非合同联系方式及签署手机号清除已用后台读取或订单维护写入演练；合同、合同当事人和验签/存证证据在专用私有 OSS 至少保留 10 年，Bucket 保留锁、下载哈希复核和后台权限已验收。
...
- [ ] 首页、四频道、OPC 下单/签署/付款、签署与支付宝返回页、其他表单与后台在 `360 / 768 / 1280 / 1440` 下完成键盘、焦点、错误、外部跳转和溢出检查。
