...
本报告记录新文档体系与 2026-07-24 时点实现的差距、已完成的安全清理和下一阶段方向。它不定义需求；需求以[文档权威索引](README.md)中的规范与 ADR 为准，逐项状态以[实现追踪矩阵](Vault2077-Implementation-Traceability.md)为准。文中旧的“每小时”节奏与历史保留表述均已由 ADR-0015 的当前快照、白天低频调度和自动处理边界取代。
...
- 审计时十项基础设施已迁移到首发基线名称与能力模型；2026-07-25 后具体项目改由受控 OPC 菜单修订，价格、周期和正式材料继续等待业务确认。
...
- rankings 改为北京时间 08:35、12:35、16:35、20:35 处理平台榜；Frontier 在 08:45–22:45 每两小时处理观察与回退。业务时钟迁移到境内受监控 scheduler，保留境内 GitHub 直连。
