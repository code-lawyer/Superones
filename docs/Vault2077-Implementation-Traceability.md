...
| 无账号线下对公转账订单 | partial | ADR-0020 运行时代码已实现：线上付款禁用、企业账户/协议 PDF/联系人二维码同页、不可静默覆盖的 PostgreSQL 付款资料修订、不缓存的资产 SHA-256 与当前修订双重匹配、内联 PDF 预览、旧资料刷新提示、流式请求体上限、固定金额订单与付款附言、必填加密通知邮箱、用户/负责人下单及到账四类幂等 outbox、同域 SMTP 门禁、下单后及刷新后的状态/凭证入口、完整服务/协议/账户/PDF/二维码快照、加密联系人和付款户名、规范化唯一流水、后台 Passkey 再认证与服务端逐项确认后的到账核验、未付款取消、90 天清理、银行付款凭证、提供商隔离、发布工具和替换手册均有代码与行为测试；新支付宝入口关闭时既有 ADR-0019 订单仍可验真、查询、关单和退款。负责人已于 2026-08-11 提供真实企业账户和联系人二维码、审阅正式版协议并授权上线测试；状态仍为 partial，直至完成同域发信服务的 DNS/SMTP 配置、四类真实邮件、浏览器与到账核验验收，且线下退款仍按人工财务流程处理 |
