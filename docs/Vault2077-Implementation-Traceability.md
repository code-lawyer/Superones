...
| 无账号线下对公转账订单 | partial | ADR-0020 运行时代码已实现：线上付款禁用、企业账户/协议 PDF/联系人二维码同页、不可静默覆盖的 PostgreSQL 付款资料修订、资产 SHA-256 与当前修订双重匹配、旧资料刷新提示、固定金额订单与付款附言、下单后及刷新后的状态/凭证入口、服务/协议/账户/PDF/二维码快照、加密联系人和付款户名、规范化唯一流水、后台 Passkey 再认证后的到账核验与未付款取消、90 天清理、银行付款凭证、outbox、提供商隔离、发布工具和替换手册均有代码与行为测试；既有 ADR-0019 支付宝订单保留。负责人已于 2026-08-11 提供真实企业账户和联系人二维码、审阅正式版协议并授权上线测试；状态仍为 partial，直至目标 VPS 完成资料发布、release 部署、邮件/浏览器/到账核验验收，且线下退款仍按人工财务流程处理 |
