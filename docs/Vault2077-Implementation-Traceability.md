...
| SiC 内容组 | partial | 正式页面已实现论文、榜单、档案、课程和播客连续聚合总览、栏目定位、轻量公开投影与服务端五条分批读取；后续请求固定所属内容组的首屏快照 ID，各组独立读取，快照变化时明确要求刷新而不混合结果，读取诊断只进入 `/pipeline`。档案来源已迁入 SiC 单一路由并由 sic 通道采集，运行时内部标识仍保留 `documents` |
...
| 可发布游骑兵身份 | done | 初始十种身份已迁入带稳定 ID 的目录 schema v2；后台支持新增、改名、排序、已发布 ID 锁定和删除空身份，顾问引用、缺失字段拒绝、v1 当前／历史快照迁移、上一 release 兼容序列化和原子发布校验已实现 |
