...
| 频道编辑配置 | partial | 双配置路由、独立并发/批量/超时/熔断、主/受控备用、无限请求额度与固定单轮时间预算及真实部署探针已实现；配置门禁拒绝错误 MiMo 域名和有限生产额度，基础设施故障不会再误标 processed；待重部署后保存真实提供方探针、容量、长积压和切换审计证据 |
...
| 生产持久化 | partial | PostgreSQL state adapter、专用 inbox/安全表、SiC 逐条发布表、版本迁移和生产 fail-closed 已实现；SiC 保留 inactive 恢复证据并兼容上一 release。真实数据库迁移、自动备份、恢复和容量证据仍需逐次留存 |
| 生产诊断边界 | done | 首页与公开频道只显示最近成功内容或中性空态，不序列化读取、采集和处理故障；具体诊断集中在受保护的 `/pipeline` 与内部健康检查 |
...
| 生产不回退演示数据 | done | 公开读取失败不回退演示数据，只显示中性空态；文件适配器仅用于非生产并明确标识为预览 |
...
| 要求 | 状态 | 说明 |
| --- | --- | --- |
| 基础设施/专项服务/游骑兵三入口 | done | `/opc` 单一工作台的三个可恢复视图已实现；基础设施和专项服务不复制详情路由，游骑兵展开头像整体链接到 `/opc/rangers/[slug]` 档案路由 |
| 固定服务分类与公开说明 | done | 三个基础设施分组、六个专项领域及九条可发布说明已实现；后台只能编辑说明，前台不再用项目数量生成分类文案 |
| 七项基础设施与十四项专项服务公开 SKU | done | 受控分类、范围、材料、成果、验收、人民币公开价、费用说明、周期、边界和稳定路由已实现；项目可经后台草稿增删改和排序 |
| 可发布游骑兵身份 | done | 初始十种身份已迁入带稳定 ID、公开名称和类别说明的目录 schema v2；后台支持新增、改名、说明修改、排序、已发布 ID 锁定和删除空身份，顾问引用、缺失字段拒绝、v1／旧 v2 当前与历史快照迁移、上一 release 兼容序列化和原子发布校验已实现；公开导航不读取个人标签生成类别说明 |
| 责任主体和联系入口区分 | partial | 需要页面文案与真实资料验收 |
| 服务目录后台 | done | 独立 OPC 管理接口支持草稿、新增、移除、排序、结构化字段编辑、完整性校验、乐观并发、二次确认、发布快照和审计；自动资讯与榜单不进入该接口 |
| 无账号线下对公转账订单 | partial | ADR-0020/0021/0022 运行时代码已实现：唯一公开付款方式为线下对公转账且不展示已退役在线支付入口；企业账户/协议 PDF/联系人二维码同页、不可静默覆盖的 PostgreSQL 付款资料修订、固定金额订单、必填加密通知邮箱与身份证号码、敏感信息单独同意、用户/负责人下单及到账 outbox、安全幂等退款申请与负责人邮件、下单后及刷新后的状态/凭证入口、完整快照、后台再认证到账与 dossier、未付款取消、清理、银行付款凭证、提供商隔离、发布工具和替换手册。负责人已提供真实企业账户和二维码并授权测试；状态仍为 partial，直至 v2 协议按新修订发布、身份证与退款申请行为／浏览器／真实邮件验收完成。线下退款付款本身仍按人工财务流程处理，申请不等于退款完成 |
...
| 要求 | 状态 | 说明 |
| --- | --- | --- |
| 四内容组 | done | 四组、连续聚合页面、稳定栏目锚点、每次五条分批展开和移动端单栏重组已实现；运行时内部标识继续使用 `documents`，公开术语为“档案” |
| OpenGithubs/HF/OpenRouter 榜单 | done | GitHub 榜经官方 REST API 获取第三方聚合结果；提供方顺序与 last-success 保留已实现，stale 只在 `/pipeline` 明示 |
...
| 要求 | 状态 | 下一证据 |
| --- | --- | --- |
| PostgreSQL 生产写模型 | partial | 7 个版本迁移、单调快照、claim token/退避、专用 inbox/审计/限速/后台会话表及 PostgreSQL 17 集成测试已实现；缺目标数据库备份恢复与容量测试 |
