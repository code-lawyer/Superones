...
| SiC 内容组 | partial | 正式页面已实现论文、榜单、档案、课程和播客连续聚合总览、栏目定位、轻量公开投影与服务端五条分批读取；后续请求固定所属内容组的首屏快照 ID，各组独立降级且快照变化时明确要求刷新而不混合结果。档案来源已迁入 SiC 单一路由并由 sic 通道采集，运行时内部标识仍保留 `documents` |
