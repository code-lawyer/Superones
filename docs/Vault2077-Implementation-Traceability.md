...
| 无账号线下对公转账订单 | partial | ADR-0020/0021 运行时代码已实现：线上付款禁用、企业账户/协议 PDF/联系人二维码同页、不可静默覆盖的 PostgreSQL 付款资料修订、固定金额订单、必填加密通知邮箱与身份证号码、敏感信息单独同意、用户/负责人下单及到账 outbox、安全幂等退款申请与负责人邮件、下单后及刷新后的状态/凭证入口、完整快照、后台再认证到账与 dossier、未付款取消、清理、银行付款凭证、提供商隔离、发布工具和替换手册。负责人已提供真实企业账户和二维码并授权测试；状态仍为 partial，直至 v2 协议按新修订发布、身份证与退款申请行为／浏览器／真实邮件验收完成。线下退款付款本身仍按人工财务流程处理，申请不等于退款完成 |
