...
| 无账号全额预付与纸质合同线上订单 | partial | ADR-0019 的线上运行时代码已实现：纸质签约可选、电子签按钮灰色禁用、加密签约方/地址、完整在线协议正文与哈希快照、固定金额支付宝收银台、付款通知/主动查询、`paid_pending_contract` 合同门禁、查询优先的稳定全额退款、动态 PNG 到账凭证、`lanzhouda@163.com` 摘要邮件 outbox 与定时任务、贯通后台 dossier、乐观并发和成功审计原子性均已有代码与测试；e 签宝 V3 与私有归档保留为未来电子签模式。状态仍为 partial，因为支付宝/邮件生产凭证、目标 PostgreSQL、systemd timer、真实交易及退款、通知丢失恢复和浏览器验收尚未在生产环境完成，门禁通过前真实付款入口保持关闭 |
