...
export function FrontierRanking({ items, emptyMessage = "本赛季尚无通过验证的项目。提交第一个公开仓库。" }: { items: FrontierEntry[]; emptyMessage?: string }) {
  return (
    <div className="record-list frontier-list">
      ...
      {items.length === 0 ? <p className="ranking-empty">{emptyMessage}</p> : null}
      {items.map((entry) => (
        <article className="record-row frontier-row" key={entry.repo}>
          ...
          <strong className="trend-value trend-value--hot mono">+{formatNumber(entry.delta)}</strong>
        </article>
      ))}
    </div>
  );
