...
test("OPC ranger shelf stays vertical, edge-aligned, and portrait-led", async () => {
  ...
  assert.match(styles, /\.opc-ranger-shelf__portrait figcaption p\s*\{[\s\S]*?max-width:\s*34ch[\s\S]*?-webkit-line-clamp:\s*2/);
...
test("OPC ranger profiles use the public dossier composition at every breakpoint", async () => {
  ...
  assert.match(profile, /className=\{`opc-ranger-dossier__hero\$\{profile\.signature \? " has-statement" : ""\}`\}/);
  ...
  assert.match(profile, /profile\.signature \? \([\s\S]*?<blockquote className="opc-ranger-dossier__statement">[\s\S]*?STATEMENT \/ 个人签名[\s\S]*?<p>\{profile\.signature\}<\/p>[\s\S]*?<\/blockquote>/);
  assert.doesNotMatch(profile, /opc-ranger-dossier__title[\s\S]*?opc-ranger-dossier__signature/);
  ...
  assert.match(admin, /label="个人签名"[\s\S]*?value=\{ranger\.signature \?\? ""\}[\s\S]*?maxLength=\{RANGER_SIGNATURE_MAX_LENGTH\}/);
  assert.match(admin, /RANGER_SIGNATURE_MAX_LENGTH/);
  ...
  assert.match(styles, /\.opc-ranger-dossier__hero\s*\{[\s\S]*?grid-template-columns:\s*var\(--dossier-spine\) minmax\(0, 1\.42fr\) minmax\(320px, \.78fr\)/);
  assert.match(styles, /\.opc-ranger-dossier__hero\s*\{[\s\S]*?grid-template-rows:\s*auto auto auto/);
  assert.match(styles, /\.opc-ranger-dossier__hero\.has-statement\s*\{[\s\S]*?grid-template-rows:\s*auto repeat\(2, minmax\(0, 1fr\)\)/);
  assert.match(styles, /\.opc-ranger-dossier__spine\s*\{[\s\S]*?grid-row:\s*1 \/ 4/);
  assert.doesNotMatch(styles, /min-height:\s*clamp\(560px,\s*58vw,\s*760px\)/);
  assert.match(styles, /\.opc-ranger-dossier__statement\s*\{[\s\S]*?grid-column:\s*2 \/ -1[\s\S]*?grid-row:\s*2[\s\S]*?border-top:\s*1px solid var\(--carbon\)/);
  assert.match(styles, /\.opc-ranger-dossier__statement > span\s*\{[\s\S]*?display:\s*flex[\s\S]*?align-items:\s*center/);
  assert.match(styles, /\.opc-ranger-dossier__statement > p\s*\{[\s\S]*?display:\s*flex[\s\S]*?align-items:\s*center[\s\S]*?max-width:\s*58ch[\s\S]*?overflow-wrap:\s*anywhere/);
  assert.match(styles, /\.opc-ranger-dossier__issue-register\s*\{[\s\S]*?grid-column:\s*2 \/ -1[\s\S]*?grid-row:\s*3[\s\S]*?grid-template-columns:\s*repeat\(3, minmax\(0, 1fr\)\)/);
  assert.match(styles, /\.opc-ranger-dossier__issue-register > div\s*\{[\s\S]*?display:\s*flex[\s\S]*?flex-direction:\s*column[\s\S]*?justify-content:\s*center/);
  ...
  assert.doesNotMatch(styles, /\.opc-ranger-dossier__signature/);
  ...
  assert.match(styles, /@media \(max-width: 820px\)[\s\S]*?\.opc-ranger-dossier__hero\s*\{[\s\S]*?grid-template-columns:\s*var\(--dossier-spine\) minmax\(0, 1fr\)[\s\S]*?grid-template-rows:\s*auto auto auto auto/);
  assert.match(styles, /@media \(max-width: 820px\)[\s\S]*?\.opc-ranger-dossier__hero\.has-statement\s*\{[\s\S]*?grid-template-rows:\s*auto auto auto auto/);
  assert.match(styles, /@media \(max-width: 820px\)[\s\S]*?\.opc-ranger-dossier__statement\s*\{[\s\S]*?grid-column:\s*2[\s\S]*?grid-row:\s*3[\s\S]*?\.opc-ranger-dossier__issue-register\s*\{[\s\S]*?grid-row:\s*4/);
